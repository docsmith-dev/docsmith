...
interface DocNavigationProps {
  ...
  children:
    | React.ReactNode
    | ((props: DocNavigationRenderProps) => React.ReactNode);
}
...
export function DocNavigation({
  ...
  return (
    <nav aria-label="Document navigation" {...props}>
      {typeof children === "function" ? children({ previous, next }) : children}
    </nav>
  );
}
...
export function DocNavigationList({
  children,
  ...props
}: DocNavigationListProps) {
  return (
    <ul role="list" {...props}>
      {children}
    </ul>
